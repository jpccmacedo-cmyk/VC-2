...
def interpretar_aba_como_data(nome_aba, ano_referencia, mes_referencia):
    ...
    Aceita:
    - D ou DD: 3, 03, 15 usando mês/ano de referência
    - DDMM: 0306, 1506 usando ano de referência
    """
...
def listar_datas_disponiveis_arquivos(arquivos_salvos, ano_referencia, mes_referencia, quantidade_datas_filtro=10):
    """
    Lista datas encontradas nas abas dos arquivos.
    Retorna as datas mais recentes em sequência contínua, dentro do mês/ano de referência.
    """
    datas_encontradas = {}
    ...
    # Mantém sequência contínua a partir da data mais recente encontrada.
    sequencia = []
    data_anterior = None
...
def encontrar_aba_por_data_ou_gerencial(wb_origem, data_selecionada, ano_referencia, mes_referencia):
    """
    Para cada data escolhida:
    1. tenta encontrar aba correspondente àquela data;
    2. se não encontrar no arquivo, usa aba Gerencial como fallback;
    3. se nenhuma existir, registra erro do arquivo.
    """
    abas_compativeis = []
...
def processar_arquivos_salvos(arquivos_salvos, data_referencia, data_selecionada, copiar_estilos=True, exibir_progresso=True):
    ...
    logs.append("Este arquivo final terá somente as abas correspondentes a esta data específica.")
    ...
    for indice, item in enumerate(arquivos_salvos, start=1):
        ...
        try:
            ...
            arquivos_processados.append({
                ...
                "criterio": f"{criterio_aba} | Data do consolidado: {data_selecionada.strftime('%d/%m/%Y')}"
            })
...
def processar_multiplas_datas(arquivos_salvos, datas_selecionadas, data_referencia, copiar_estilos=True):
    ...
    - se o usuário escolher 03/06 e 04/06;
    - o sistema gera dois arquivos finais separados;
    - o arquivo de 03/06 contém somente as abas consolidadas referentes a 03/06;
    - o arquivo de 04/06 contém somente as abas consolidadas referentes a 04/06.
    """
    resultados = []
...
st.markdown(
    ...
    **Exemplo:**
    ...
    - Se você selecionar **03/06** e **04/06**;
    - O sistema gera **2 planilhas consolidadas**;
    - Uma planilha final somente com as abas da data **03/06**;
    - Outra planilha final somente com as abas da data **04/06**.

    Cada consolidado mantém as regras de nomes, layout, limpeza da coluna L em diante e ordem das abas.
    """
)
...
st.sidebar.caption("O filtro mostra datas consecutivas dentro do mês/ano de referência.")
...
st.caption(
    ...
    "Exemplo: selecionar 03/06 e 04/06 gera um Excel somente para 03/06 e outro somente para 04/06. "
