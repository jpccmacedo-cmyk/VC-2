...
def identificar_tipo_aba(nome_arquivo):
    ...
    if "gerencial_" in nome:
        return "XAM"
    if re.search(r"gerencial\s\d{2}-\d{2}-\d{4}", nome):
        return "COB"
    if re.search(r"gerencial\s\d{1,2}", nome):
        return "PVE"
...
def limpar_coluna_l(aba):
    ...
    for row in aba.iter_rows(min_row=1, max_row=aba.max_row, min_col=COLUNA_L, max_col=aba.max_column):
        for c in row:
