...
def obter_data_real_da_aba(aba, ano_referencia, mes_referencia):
    ...
    Se a aba for Gerencial, usa J3/K3.
    """
...
# ============================================================
# ESCOLHA E ÍNDICE DE DATAS — REGRA PRINCIPAL
# ============================================================
...
def listar_registros_validos_do_arquivo(wb, ano_referencia, mes_referencia):
    ...
    for nome_aba in wb.sheetnames:
        ...
        if data_real:
            ...
            prioridade = 1 if data_nome == data_real else (3 if normalizar_texto(nome_aba) == normalizar_texto(NOME_ABA_GERENCIAL) else 2)
            registros.append({"aba": nome_aba, "data": data_real, "prioridade": prioridade})
    return registros


def escolher_aba_principal_do_arquivo(wb, ano_referencia, mes_referencia):
    """
    Cada planilha de origem deve contribuir para uma única data e uma única aba final.
    Se o arquivo contém histórico, usa somente a MAIOR data encontrada nesse arquivo.
    Assim, um arquivo de CUI 06/06 que também tem 04/06 e 05/06 só entra no consolidado 06/06.
    """
...
def escolher_aba_de_consolidado_existente(wb, data_processar, ano_referencia, mes_referencia, nome_arquivo):
    """Para consolidado parcial, reaproveita abas padrão da data selecionada."""
    ...
    for nome_aba in wb.sheetnames:
        ...
        if nome_padrao not in ORDEM_ABAS:
            continue
        data_aba = obter_data_real_da_aba(wb[nome_aba], ano_referencia, mes_referencia)
        if data_aba == data_processar or (data_aba is None and data_nome_arquivo == data_processar):
            registros.append(nome_aba)
    return registros
...
def gerar_chave_cache_datas(arquivos_salvos, ano_referencia, mes_referencia):
    ...
    return (tuple(assinatura), ano_referencia, mes_referencia, "1-arquivo-1-data-principal")


def construir_indice_datas(arquivos_salvos, ano_referencia, mes_referencia, quantidade_datas_filtro):
    """
    Lista datas disponíveis usando a data principal de cada arquivo de origem.
    Isso evita que histórico interno crie abas em mais de uma data.
    """
    ...
    for idx, item in enumerate(arquivos_salvos, start=1):
        ...
        try:
            ...
            principal_arquivo[caminho] = {"data": data_principal, "aba": registro["aba"], "tipo": "origem"}
...
def processar_data(data_processar, arquivos_salvos, principal_arquivo, data_referencia, copiar_estilos=True, callback_progresso=None):
    ...
    logs.append("Também permite enviar consolidado parcial com planilhas faltantes.")
    ...
    # 2) Planilhas de origem/faltantes: só entram se a data principal do arquivo for a data selecionada.
    for item in arquivos_salvos:
        if detectar_consolidado_existente(item):
            continue
        wb = None
        try:
            ...
            limpar_conteudos_a_partir_da_coluna_m(aba_destino)
            aplicar_configuracoes_finais_aba(aba_destino)
            arquivos_processados.append({"arquivo": item["nome"], "aba_origem": nome_aba_origem, "aba_final": nome_padrao, "criterio": f"Data principal do arquivo = {data_processar.strftime('%d/%m/%Y')}"})
...
st.markdown(
    ...
    **Regra principal:** cada consolidado tem somente abas da data selecionada.  
    Exemplo: se você enviar 21 planilhas de 3 datas diferentes, o sistema deve gerar 3 consolidados, cada um com até 7 abas: `COB`, `CUI`, `EDE`, `NOB`, `PVE`, `SOB`, `XAM`.

    Também é possível enviar um **consolidado parcial** junto com planilhas faltantes para gerar o consolidado final.
    """
)
st.divider()
...
st.divider()
st.caption("Cada consolidado contém apenas abas da data selecionada. Arquivos com histórico interno entram somente na data principal do próprio arquivo. Ordem final: COB, CUI, EDE, NOB, PVE, SOB, XAM.")
